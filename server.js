...
const opcodeVectors = {
    3: { getGlobal: 0xA4, loadK: 0x9F, nameCall: 0x52, close: 0x1E, jump: 0x4D, getUpvalue: 0x12, setUpvalue: 0x13, getTableKs: 0x4A, setTableKs: 0x4B, call: 0x51, add: 0x17, sub: 0x18, mul: 0x19, div: 0x1A },
    4: { getGlobal: 0xB2, loadK: 0xA1, nameCall: 0x58, close: 0x22, jump: 0x51, getUpvalue: 0x15, setUpvalue: 0x16, getTableKs: 0x4E, setTableKs: 0x4F, call: 0x57, add: 0x1B, sub: 0x1C, mul: 0x1D, div: 0x1E },
    5: { getGlobal: 0xC1, loadK: 0xB5, nameCall: 0x61, close: 0x2A, jump: 0x5D, getUpvalue: 0x19, setUpvalue: 0x1A, getTableKs: 0x57, setTableKs: 0x58, call: 0x60, add: 0x21, sub: 0x22, mul: 0x23, div: 0x24 },
    6: { getGlobal: 0xD4, loadK: 0xC2, nameCall: 0x6E, close: 0x31, jump: 0x65, getUpvalue: 0x20, setUpvalue: 0x21, getTableKs: 0x63, setTableKs: 0x64, call: 0x6D, add: 0x28, sub: 0x29, mul: 0x2A, div: 0x2B }
};
...
function decodeLegacyLua(buffer) {
    try {
        let ptr = 12; // Skip standard 12-byte 5.1 metadata header profile
        
        let srcSize = buffer.readUInt32LE(ptr);
        ptr += 4;
        let sourceName = buffer.toString('utf8', ptr, ptr + srcSize - 1);
        ptr += srcSize;

        ptr += 12; // Skip line definition metadata bounds

        let sizeCode = buffer.readUInt32LE(ptr);
        ptr += 4;
        ptr += sizeCode * 4; // Advance past raw opcode allocations

        let sizeConsts = buffer.readUInt32LE(ptr);
        ptr += 4;

        let stringTable = [];
        for (let i = 0; i < sizeConsts; i++) {
            let type = buffer[ptr++];
            if (type === 4) { // String Type tag indicator constant
                let strLen = buffer.readUInt32LE(ptr);
                ptr += 4;
                let stringVal = buffer.toString('utf8', ptr, ptr + strLen - 1);
                stringTable.push(stringVal);
                ptr += strLen;
            }
        }

        let output = `-- Legacy Roblox Bytecode Profile (Lua 5.1 Standard)\n`;
        output += `-- Origin Source: ${sourceName}\n\n`;
        
        if (stringTable.includes("print")) {
            let arguments = stringTable.filter(s => s !== "print");
            output += `print("${arguments.join('", "')}")\n`;
        } else {
            output += `-- Reconstructed Constants:\n`;
            stringTable.forEach(s => output += `-- "${s}"\n`);
        }
        return output;
    } catch (err) {
        return "-- Error: Failed parsing legacy block data frame";
    }
}
        ...
        for (let i = 0; i < sizeCode; i++) {
            ...
            if (op === vectors.getGlobal || op === vectors.loadK) {
                ...
                if (funcVar.includes(":GetService") && args.length > 0) {
                    const cleanService = args.replace(/"/g, '');
                    const serviceVar = cleanService.charAt(0).toLowerCase() + cleanService.slice(1);
                    linesOfCode.push(`    local ${serviceVar} = game:GetService(${args})`);
...
function decompileLuau(hexString, callback) {
    try {
        ...
        if (buffer[0] === 0x1B && buffer[1] === 0x4C && buffer[2] === 0x75 && buffer[3] === 0x61) {
            return callback(decodeLegacyLua(buffer));
        }
